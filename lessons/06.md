...
Our coworkers have started on a basic frontend, but we'll need to do the work to deliver the data to the views. But we'll need to cover some foundational info before we get started.

You probably have an intuitive understanding of how the internet works, but it's important to understand the language of the web at a deeper level if you plan to develop web applications. Watch this video for a brief, but thorough overview. https://youtu.be/7_LPdttKXPc?si=yI7dYDs1HAix5qz6
...
In the directory `app/controllers/`, (TODO - is the controller file present? remove it from app) create a new file called `trimesters_controller.rb` as per the naming conventions for controllers described in [Rails Guides here](https://guides.rubyonrails.org/action_controller_overview.html#controller-naming-convention).
